...
const allTokens = [
  ...
  // catch-all for any remaining single characters we don't need to parse semantically
];
...
export interface DecisionNode {
  ...
  stateQuerySources: string[]; // all FROM sources in STATE AS (including subqueries)
  comment?: string;
}
...
export function parseDeQL(input: string): ParseResult {
  ...
  // Collect comments with offset and type info
  ...
  const maxPos = tokens.length; // safety bound
  ...
  // Guard: ensure pos always advances in loops
  ...
  function parseEventEmitList(): string[] {
    ...
    // Skip remaining WHERE clause until semicolon
    while (pos < tokens.length && !match(Semicolon)) {
      pos++;
    }
    // Safety: if we haven't moved at all, force advance
  ...
  function parseCreateDecision(): DecisionNode | null {
    ...
    // STATE AS (optional) — collect ALL FROM sources including subqueries
    if (match(State)) {
      consume();
      if (match(As)) consume();
      hasStateQuery = true;
      while (pos < tokens.length && !match(Emit)) {
        if (match(From)) {
          ...
          if (src) {
            ...
            if (!stateQuerySource) stateQuerySource = src; // first one is primary
          }
          continue;
        }
        pos++;
  ...
  function parseCreateProjection(): ProjectionNode | null {
    ...
    // Parse through the SELECT body until semicolon
    while (pos < tokens.length && !match(Semicolon)) {
      if (match(From)) {
        consume();
  ...
  function parseInspectDecision(): InspectDecisionNode | null {
    // INSPECT and DECISION already consumed by main loop
  ...
  function parseInspectProjection(): InspectProjectionNode | null {
    // INSPECT and PROJECTION already consumed by main loop
  ...
  function parseCreateTable(): CreateTableNode | null {
    // TABLE already consumed by main loop
    ...
    while (pos < tokens.length && match(LParen)) {
      ...
      while (pos < tokens.length && !match(RParen)) {
        if (match(StringLiteral) || match(NumberLiteral)) {
          ...
          // skip unexpected token inside row
          pos++;
        }
      }
      if (match(RParen)) consume(); // )
      values.push(row);
      if (match(Comma)) consume(); // comma between rows
    }
    if (match(Semicolon)) consume();
  ...
  // ─── Main parse loop ─────────────────────────────────────────────────────────
  let loopGuard = 0;
  const maxIterations = tokens.length * 2; // generous safety bound
  while (pos < tokens.length) {
        ...
        try {
          ...
          if (node) {
            ...
            sourceFragments.push({
              nodeKind: node.kind,
              nodeName: node.decisionName,
              startOffset: inspectStartOffset,
              endOffset: getLastEndOffset(),
              startLine: inspectStartLine,
              endLine: getLastEndLine(),
            });
          }
        } catch (e) {
      ...
      try {
        if (tokenMatcher(next, Table)) {
          ...
          if (node) {
            ...
            sourceFragments.push({
              nodeKind: node.kind,
              nodeName: node.tableName,
              startOffset: createStartOffset,
              endOffset: getLastEndOffset(),
    ...
    // Safety: ensure pos always advances
    if (pos === prevPos) pos++;
  }
