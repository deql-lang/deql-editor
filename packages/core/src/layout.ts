...
/**
 * Main layout function. Positions groups left-to-right in order,
 * then stacks nodes vertically within each group.
 */
export async function applyLayout(graphIR: GraphIR, layoutMode: DiagramLayout = 'compact'): Promise<LayoutResult> {
  ...
  function computeGroupLayout(memberNodes: GraphNode[]) {
    ...
    return { width: totalWidth, height: maxColHeight + sp.nodePadTop + sp.nodePadBottom, colLayouts };
  }
  ...
  for (const group of aggGroupsList) {
    const memberNodes = group.nodeIds.map(id => graphIR.nodes.find(n => n.id === id)).filter((n): n is GraphNode => n !== undefined);
    ...
    for (const colLayout of layout.colLayouts) {
      for (const nl of colLayout.nodes) {
        ...
        if (idx >= 0) positioned[idx] = { ...positioned[idx], position: { x: nodeX, y: nodeY } };
      }
    }

    groupBounds.push({ id: group.id, label: group.label, x: groupX, y: groupY, width: layout.width, height: layout.height });
    for (const nodeId of group.nodeIds) nodeGroupMap[nodeId] = group.id;
  ...
  if (projGroup) {
    const memberNodes = projGroup.nodeIds.map(id => graphIR.nodes.find(n => n.id === id)).filter((n): n is GraphNode => n !== undefined);
    if (memberNodes.length > 0) {
      const layout = computeGroupLayout(memberNodes);
      const projX = aggStartX + maxAggWidth + sp.groupGap;
      const totalAggHeight = cursorY - sp.groupGap - 30;
      const projY = 30 + Math.max(0, (totalAggHeight - layout.height) / 2);
      ...
      groupBounds.push({ id: projGroup.id, label: projGroup.label, x: projX, y: projY, width: layout.width, height: layout.height });
      for (const nodeId of projGroup.nodeIds) nodeGroupMap[nodeId] = projGroup.id;
    }
  }

  if (inspGroup) {
    const memberNodes = inspGroup.nodeIds.map(id => graphIR.nodes.find(n => n.id === id)).filter((n): n is GraphNode => n !== undefined);
    if (memberNodes.length > 0) {
      const inspSp = { nodePadSide: sp.nodePadSide + 20, nodePadTop: sp.nodePadTop + 10, nodePadBottom: sp.nodePadBottom + 10, nodeGapX: sp.nodeGapX + 30, nodeGapY: sp.nodeGapY + 15 };
      ...
      groupBounds.push({ id: inspGroup.id, label: inspGroup.label, x: inspX, y: inspY, width: inspWidth, height: inspHeight });
      for (const nodeId of inspGroup.nodeIds) nodeGroupMap[nodeId] = inspGroup.id;
    }
  }
  ...
  if (ungroupedNodes.length > 0) {
    ...
    for (const node of ungroupedNodes) {
      ...
      if (idx >= 0) positioned[idx] = { ...positioned[idx], position: { x: ungroupedX, y: rowY } };
      rowY += 120;
    }
  }
...
function applyFlatLayout(graphIR: GraphIR, layoutMode: DiagramLayout): LayoutResult {
  ...
  for (const colKey of [...columns.keys()].sort()) {
    ...
    for (const node of colNodes) {
      const idx = positioned.findIndex(n => n.id === node.id);
      if (idx >= 0) positioned[idx] = { ...positioned[idx], position: { x: colX, y: rowY } };
      rowY += 120;
    }
    colX += 250;
  }
